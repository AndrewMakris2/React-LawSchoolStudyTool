...
router.post("/generate", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const raw = await chatCompletion(messages, req.apiKey, { temperature: 0.9, maxTokens: 1024 });
...
router.post("/grade", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const raw = await chatCompletion(messages, req.apiKey, { temperature: 0.3, maxTokens: 1500 });
...
router.get("/history", async (req: Request, res: Response, next: NextFunction) => {
  try {
    res.json(await getDrillAttempts(req.userId));
  } catch (err) { next(err); }
});
