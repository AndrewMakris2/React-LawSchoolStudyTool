...
const NAV_GUIDES = [
  { to: "/texas-tech", icon: GraduationCap, label: "Texas Tech Guide" },
];
...
export function Sidebar({ onRequestChangeKey, onOpenSearch }: SidebarProps) {
  return (
    <aside className="no-print w-56 shrink-0 bg-gray-950 border-r border-gray-800 flex flex-col h-screen sticky top-0">
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto">
        ...
        <div className="border-t border-gray-800 pt-3 mt-3">
          ...
          <div className="space-y-1">
            {NAV_GUIDES.map(({ to, icon: Icon, label }) => (
              <NavLink
                key={to}
                to={to}
                className={({ isActive }) =>
